...
from meena import Meena
from arg import ModelConfig
from dataset import DatasetForSeq2seqV2, DatasetForSeq2seqConversation


class MeenaTrainer(object):
    def __init__(self, dataset, model, tokenizer, max_len, model_name, checkpoint_path, device=None, train_batch_size=8,
                 eval_batch_size=None, log_dir='logs', fp16=True):
    ...
    def build_dataloaders(self, train_test_split=0.1, train_shuffle=True, eval_shuffle=True):
        ...
        # logging.info(
        #     f'''train_dataloader size: {len(train_loader)} | shuffle: {train_shuffle},eval_dataloader size: {len(eval_loader.dataset)} | shuffle: {eval_shuffle}''')
        return train_loader, eval_loader

    def train(self, epochs, train_dataloader, eval_dataloader, optimizer, log_steps, ckpt_steps,
        ...
        for epoch in range(start_epoch, epochs):  # tqdm(range(epochs), desc='Epochs', position=0):
            ...
            for step, batch in pb:
                ...
                encoder_input_ids, decoder_input_ids, encoder_input_mask, labels = encoder_input_ids.to(
                    self.device), decoder_input_ids.to(self.device), encoder_input_mask.to(self.device), labels.to(
                    self.device)
                output = self.model(encoder_input_ids, decoder_input_ids, encoder_input_mask,
                                    labels)  # output: lm_logits, loss, encoder_logit, x
                loss = output[1]
    ...
    def evaluate(self, dataloader):
        ...
        for step, batch in tqdm(enumerate(dataloader), desc='Evaluating', leave=True, total=len(dataloader),
            ...
            with open('logs/meena_test_eval_results.txt', 'a+') as results_file:
                results_file.write(
                    f'{datetime.now()} | Step: {step} | Eval Loss: {total_eval_loss} | Perplexity: {total_perplexity}\n')
                results_file.close()
...
def main():
    ...
    log_dir = (f'{base_path}/lo'
               f'3gs')
    config_path = 'meena-finetuning-config-v3.json'
    ...
    optimizer = Adafactor(model.parameters(),
                          ...
                          # (default: True) if True, learning rate is scaled by root mean square of parameter
                          relative_step=False,  # (default: True) if True, time-dependent learning rate is computed
                          warmup_init=False,
                          # (default: False) time-dependent learning rate computation depends on whether warm-up initialization is being used
                          lr=5e-5)
    # optimizer = AdamW(model.parameters(), lr=3e-4)

    # if config.fp16:
    #     model, optimizer = amp.autocast(model, optimizer)
